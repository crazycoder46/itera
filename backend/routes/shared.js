...
const requirePremium = async (req, res, next) => {
  try {
    const result = await db.query('SELECT is_premium FROM users WHERE id = $1', [req.user.id]);
...
router.get('/my-code', auth, requirePremium, async (req, res) => {
  try {
    const result = await db.query('SELECT share_code FROM users WHERE id = $1', [req.user.id]);
...
router.post('/share-note', auth, requirePremium, async (req, res) => {
  try {
    ...
    // Notu getir
    const noteResult = await db.query('SELECT * FROM notes WHERE id = $1 AND user_id = $2', [noteId, req.user.id]);
    ...
    await db.query(
      ...
      [req.user.id, receiverId, noteId, note.title, note.content, note.box_type]
    );
...
router.post('/accept-note/:id', auth, requirePremium, async (req, res) => {
  try {
    ...
    await db.query(
      ...
      [req.user.id, sharedNote.title, sharedNote.content, sharedNote.box_type]
    );
